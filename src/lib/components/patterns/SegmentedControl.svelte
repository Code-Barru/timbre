<script lang="ts">
	...
	let trackCls = $derived(shape === 'pill' ? 'rounded-full p-[3px]' : 'rounded-md p-1');
	let segShapeCls = $derived(shape === 'pill' ? 'rounded-full' : 'rounded');
	let sizeCls = $derived(size === 'sm' ? 'px-3 py-1.5 text-caption' : 'px-4 py-2 text-label');
...
<div role="tablist" class="inline-flex w-fit bg-chip {trackCls} {className}">
